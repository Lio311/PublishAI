...
export default function DynamicBackground() {
    ...
    return (
        ...
        >
            ...
            <motion.div style={{ x: shouldReduceMotion ? 0 : blob1X, y: shouldReduceMotion ? 0 : blob1Y }} className="absolute top-0 left-0 w-[50vw] h-[50vw] max-w-[800px] max-h-[800px]">
                <motion.div 
                    ...
                    transition={shouldReduceMotion ? undefined : { repeat: Infinity, duration: 15, ease: "linear" }}
                    className="w-full h-full bg-blue-300 rounded-full mix-blend-multiply filter blur-[120px] opacity-40"
                />
            </motion.div>

            <motion.div style={{ x: shouldReduceMotion ? 0 : blob2X, y: shouldReduceMotion ? 0 : blob2Y }} className="absolute bottom-0 right-0 w-[60vw] h-[60vw] max-w-[900px] max-h-[900px]">
                <motion.div 
                    animate={shouldReduceMotion ? undefined : { 
                        x: [0, -150, 100, 0],
                        y: [0, -100, 100, 0],
                    ...
                    transition={shouldReduceMotion ? undefined : { repeat: Infinity, duration: 20, ease: "linear" }}
                    className="w-full h-full bg-blue-300 rounded-full mix-blend-multiply filter blur-[150px] opacity-40"
                />
            </motion.div>

            <motion.div style={{ x: shouldReduceMotion ? 0 : blob3X, y: shouldReduceMotion ? 0 : blob3Y }} className="absolute top-1/2 left-1/4 w-[40vw] h-[40vw] max-w-[600px] max-h-[600px]">
                <motion.div 
                    animate={shouldReduceMotion ? undefined : { 
                        x: [0, 50, -150, 0],
                        y: [0, 150, -50, 0],
